...
//   }
  ...
  async getColumnSelectOptions(column, { model, brand, year, keyWord }: { model?: string, brand?: string, year?: any, keyWord?: string }) {
    ...
    const parameters = [`%${model || ''}%`,`%${brand || ''}%`,`%${year || ''}%`,`%${keyWord || ''}%`,`%${keyWord || ''}%`,`%${keyWord || ''}%`,];
