  ...
  pageQueryOp: {
    ...
    where: async (ctx: Context) => {
      ...
      return [
        ...
        bodyAndDoors
          ? [
              'bodyAndDoors like :bodyAndDoors',
              { bodyAndDoors: `%${bodyAndDoors}%` },
            ]
          : [],
      ];
    },
...
export class VehicleDataController extends BaseController {
  ...
  async upload() {
    ...
    const result = excelToJson({
      sourceFile:
        __dirname +
        '\\Australia-Car-Database-make-model-year-badge-specs-handle.xlsx',
      header: {
  ...
  async getColumnSelectOptions(
    ...
    const promiseArr = Object.keys(options).map(column =>
      this.vehicleDataService
        .getColumnSelectOptions(column, {
          filters: { model, year, make, bodyAndDoors, badge, series, keyWord },
        })
        .then(res => {
          options[column] = res;
        })
