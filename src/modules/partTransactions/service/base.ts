...
export class PartTransactionsService extends BaseService {
  ...
  async getPartInfoFilterOpts() {
    ...
    .leftJoinAndSelect(CarWreckedEntity, "car_wrecked", "part_transactions.carWreckedID = car_wrecked.id")
    .select(["part_transactions.*", "car_wrecked.disassmblingInformation", "car_wrecked.containerNumber"])
    ...
    opts.partInfo = Array.from(new Set(results.map((result: any) => result.car_wrecked_disassmblingInformation)));
    opts.containerNumber = Array.from(new Set(results.map((result: any) => result.car_wrecked_containerNumber)));
